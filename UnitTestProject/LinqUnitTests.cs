...
namespace UnitTestProject
{
    ...
    public class LinqUnitTests
    {
        ...
        {
            var result = transactions
                            .Count(trans => trans.Date.DayOfWeek == DayOfWeek.Sunday);
        ...
        {
            var result = transactions.Average(trans => trans.Quantity);
        ...
        {
            var result = transactions
                            .Where(trans => trans.ProductName.Equals("Chips"))
                            .Sum(trans => trans.Quantity);
        ...
        {
            var result = transactions
                            .Where(trans => trans.ProductName.Equals("Chips") && trans.UserName.Equals("Jason"))
                            .Sum(trans => trans.Quantity);
        ...
        public void Test_HowManyBagsOfChipsDidJasonBuyIn2015()
        {
            var result = transactions
                            .Where(trans => trans.ProductName.Equals("Chips")
                                        && trans.UserName.Equals("Jason")
                                        && trans.Date.Year == 2015)
                            .Sum(trans => trans.Quantity);
        ...
        {
            var result = transactions
                            .Where(trans => trans.Date.Hour == 12)
                            .GroupBy(trans => trans.ProductName, trans => trans.Quantity, (key, values) => new { ProductName = key, Quantities = values.Sum() })
                            .OrderByDescending(group => group.Quantities).First().ProductName;
