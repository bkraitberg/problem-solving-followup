...
namespace UnitTestProject
{
    ...
    public class LinqUnitTests
    {
        ...
        {
            var result = transactions
                .Where(transaction => transaction.Date.Year == 2016)
                .Sum(transaction => transaction.Quantity);
        ...
        {
            var result = transactions
                .Where(transaction => transaction.Date.CompareTo(new DateTime(2016, 8, 29)) <= 0 && transaction.Date.CompareTo(new DateTime(2016, 8, 23)) >= 0)
                .Sum(transaction => transaction.Quantity);
        ...
        {
            var result = transactions
                .Count(transaction => transaction.Quantity > 1);
        ...
        {
            var result = transactions
                .Count(transaction => transaction.Date.DayOfWeek == DayOfWeek.Sunday);
        ...
        {
            var result = transactions
                .Average(transaction => transaction.Quantity);
        ...
        {
            var result = transactions
                .Where(transaction => transaction.ProductName.Equals("Chips"))
                .Sum(transaction => transaction.Quantity);
        ...
        {
            var result = transactions
                .Where(transaction => transaction.UserName.Equals("Jason") && transaction.ProductName.Equals("Chips"))
                .Sum(transaction => transaction.Quantity);
        ...
        {
            var result = transactions
                .Where(transaction => transaction.UserName.Equals("Jason") && transaction.ProductName.Equals("Chips") && transaction.Date.Year == 2015)
                .Sum(transaction => transaction.Quantity);
        ...
        {
            var result = transactions
                .Where(transaction => transaction.UserName.Equals("Jason") && transaction.ProductName.Equals("Chips") && transaction.Date.Year == 2016 && transaction.Date.Month == 5)
                .Sum(transaction => transaction.Quantity);
        ...
        {
            var result = transactions
                .Where(transaction => transaction.Date.Hour >= 12 && transaction.Date.Hour <= 13)
                .GroupBy(transaction => transaction.ProductName, (key, values) => new { ProductName = key, Count = values.Sum(t => t.Quantity) })
        ...
        {
            var result = transactions
                .Where(transaction => transaction.ProductName.Equals("Candy"))
                .GroupBy(transaction => transaction.UserName, (key, values) => new { UserName = key, Count = values.Sum(t => t.Quantity) })
                .OrderBy(group => group.Count)
                .Last().UserName;
        ...
        {
            var result = transactions
                .Sum(transaction => transaction.Quantity * products.Single(product => product.Name.Equals(transaction.ProductName)).Price);
        ...
        {
            var result = transactions
                .GroupBy(transaction => transaction.UserName, (key, values) => new { UserName = key, Spent = values.Sum(t => t.Quantity * products.Single(product => product.Name.Equals(t.ProductName)).Price)})
                .OrderBy(group => group.Spent)
                .Last().UserName;
        ...
        {
            var result = users.Single(u => u.Name.Equals(
                transactions
                    .GroupBy(transaction => transaction.UserName, (key, values) => new { UserName = key, Spent = values.Sum(t => t.Quantity * products.Single(product => product.Name.Equals(t.ProductName)).Price) })
                    .OrderBy(group => group.Spent)
                    .Last().UserName)
