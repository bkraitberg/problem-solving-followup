...
namespace UnitTestProject
{
    ...
    public class LinqUnitTests
    {
        ...
        {
            var result = transactions.Where(a => a.Date.Year == 2016).Sum(x => x.Quantity); // TODO
        ...
        {
            var result = transactions.Where(a => a.Date >= DateTime.Now.AddDays(-8)).Sum(x => x.Quantity); ; // TODO
        ...
        {
            var result = transactions.Where(a => a.ProductName == "Chips").Sum(x => x.Quantity); // TODO
        ...
        {
            //var result = transactions.GroupBy(p => p.ProductName).Max(x => x.)  ; // TODO
            var result = transactions.GroupBy(a => new { a.ProductName, a.Date, a.Quantity })
                           ...
                           .OrderBy(r => r.Key.ProductName)//.Max();
        ...
        {
            var result = transactions.GroupBy(a => new { a.ProductName, a.Quantity })
                           .OrderByDescending(r => r.Sum(b => b.Quantity))
                           .Last().Key.ProductName;
            //.FirstOrDefault().Key.ProductName; // TODO
        ...
        {
            var result = transactions.GroupBy(a => new { a.UserName, a.ProductName, a.Quantity })
                          .Where(r => r.Key.ProductName == "Candy")
                          .OrderByDescending(r => r.Key.Quantity)
                          .FirstOrDefault().Key.UserName;

            Assert.AreEqual("David", result);   // TODO
        }
        ...
        //{
        //   var result = transactions.GroupBy(a => new { a.UserName, a.ProductName, a.Quantity }); // TODO
